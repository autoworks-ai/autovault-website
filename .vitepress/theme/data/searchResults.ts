...
export const searchResults: TopbarSearchResult[] = [
  { title: "Quick start", section: "Get started", href: "/quick-start", terms: "install local vault doctor first skill scope run vault anatomy add-local remove uninstall autovault_skill_install bootstrap setup wizard adoption augment backup in-place" },
  ...
  { title: "API reference", section: "Reference", href: "/api", terms: "cli library http mcp endpoint load render verify resolve add_skill update_skill delete_skill propose_skill get_skill check_updates setup remove uninstall audit-repo import-autohub skill list skill which environment variables AUTOVAULT_STORAGE_PATH AUTOVAULT_MODE" },
  ...
  { title: "Troubleshooting", section: "Reference", href: "/troubleshooting", terms: "troubleshooting faq setup wizard adoption augment backup in-place no tty no terminal sync-profiles ENOENT doctor repair signature mismatch remove uninstall no-discover link skill which import didn't install agent shell tool" },
  ...
  { title: "Changelog", section: "Reference", href: "/changelog", terms: "release notes remote mcp oauth add-local remove uninstall transforms resource drift" }
];
